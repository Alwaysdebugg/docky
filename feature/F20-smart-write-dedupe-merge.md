...
- done — 2026-06-18 实现并通过测试(210/210)。
  ...
  - 加固(2026-06-23):**agent 新建文档自动加日期前缀**。`src/core.ts` 新增 `datePrefixed(name)`(`YYYY-MM-DD-HHmm-` 前缀,UTC,与 `today()`/append 戳一致;以 `DATE_PREFIX_RE` 检测已带 ISO 日期的名字 → **幂等** 不重复加戳);`smartWrite` 抽出 `createNew()`,**仅在真正新建文档的分支**(`new` 无冲突 / `append`·`replace` 目标不存在 / `merge` 无近似)套用 `datePrefixed` 并返回 dated `rel`;命中既有文档的 `append`/`replace`/`merge` 保持其**原文件名**不动(按名匹配语义不破)。`src/mcp.ts` 的 `write_doc` 描述与 `name` 字段改为引导 agent 传朴素 slug、由 docky 加戳。测试:`test/core.test.ts` 新增 describe "agent date-stamped doc names (F20)"(幂等 / 新建加戳并落 dated rel / 不二次加戳 / replace 既有不改名),并将 F22·F20 中按未加戳名断言的用例改用返回 `rel`(全量 242/242 绿)。
