...
describe("MCP resources & prompts (F25)", () => {
  it("lists documents as docky:// resources", () => {
    ...
    expect(res.some((r) => r.uri === "docky://p/design/auth.md")).toBe(true);
    expect(res.every((r) => r.mimeType === "text/markdown")).toBe(true);
  });

  it("parseDockyUri splits project and rel", () => {
    expect(parseDockyUri("docky://p/design/auth.md")).toEqual({ project: "p", rel: "design/auth.md" });
    expect(parseDockyUri("not-a-docky-uri")).toBe(null);
  });

  it("readResource reads in scope and refuses path traversal", () => {
    expect(readResource(vault, "docky://p/design/auth.md").text).toContain("important body");
  ...
  it("contextPromptMessages wraps get_context (F07)", () => {
    ...
    expect(msgs[0].content.text).toContain("design/auth.md");
  });

  it("scaffoldPromptMessages wraps the type template (F06)", () => {
    const msgs = scaffoldPromptMessages(vault, "debug", "p", "登录超时");
    expect(msgs[0].content.text).toContain("复现步骤"); // debug template section
